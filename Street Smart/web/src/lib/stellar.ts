...
/** Stream payments for a given address. Returns an "unsubscribe" function. */
export function streamPayments(
  ...
  return horizonServer
    ...
    .cursor('now')
    .stream({
      onmessage: (payment) => onPayment(payment),
      onerror: (error) => console.error('Payment stream error:', error),
    });
}
